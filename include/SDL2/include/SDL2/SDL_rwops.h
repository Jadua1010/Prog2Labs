 ...
 * # CategoryRWOPS
 *
 * This file provides a general interface for SDL2 to read and write data
 ...
 * library's fopen(), even if SDL2 doesn't happen to use fopen() behind the
 * scenes.
 *
 ...
 *
 * Closing the SDL_RWops will close the file handle SDL2 is holding internally.
 *
 ...
 * `FILE*`, depending on what system headers are available to SDL2. It is
 ...
 * implementations in SDL2, like SDL_RWFromFile() or SDL_RWFromMem(), etc.
 *
 * You must free the returned pointer with SDL_FreeRW(). Depending on your
 * operating system and compiler, there may be a difference between the
 * malloc() and free() your program uses and the versions SDL2 calls
 ...
 * their own SDL_RWops implementation. If you just need a SDL_RWops to
 * read/write a common data source, you should use the built-in
 * implementations in SDL2, like SDL_RWFromFile() or SDL_RWFromMem(), etc, and
 ...
 *
 * Prior to SDL2 2.0.10, this function was a macro.
 *
 ...
 * The data should be freed with SDL_free().
 *
 * Prior to SDL2 2.0.10, this function was a macro wrapping around
 * SDL_LoadFile_RW.
 *
 ...
 *
 * SDL2 byteswaps the data only if necessary, so the data returned will be in
 ...
 * little-endian data.
 *
 * SDL2 byteswaps the data only if necessary, so the application always
