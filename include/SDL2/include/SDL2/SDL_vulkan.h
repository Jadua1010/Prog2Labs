 ...
 * # CategoryVulkan
 *
 * Header file for functions to creating Vulkan surfaces on SDL2 windows.
 */
 ...
 *        is compatable with Tizen's implementation of Vulkan in SDL2.
 */
/* @{ */
 ...
 * of explicitly loading it at run time. This will work with SDL2 provided the
 * application links to a dynamic library and both it and SDL2 use the same
 * search path.
 *
 ...
 *
 * On Apple devices, if `path` is NULL, SDL2 will attempt to find the
 * `vkGetInstanceProcAddr` address within all the Mach-O images of the current
 * process. This is because it is fairly common for Vulkan applications to
 * link with libvulkan (and historically MoltenVK was provided as a static
 * library). If it is not found, on macOS, SDL2 will attempt to load
 * `vulkan.framework/vulkan`, `libvulkan.1.dylib`,
 * `MoltenVK.framework/MoltenVK`, and `libMoltenVK.dylib`, in that order. On
 * iOS, SDL2 will attempt to load `libMoltenVK.dylib`. Applications using a
 ...
 *
 * The `window` parameter is currently needed to be valid as of SDL2 2.0.8,
