 ...
 *
 * By default, SDL2 emulates Alt+Tab functionality while the keyboard is
 ...
 *
 * - "0": SDL2 will not handle Alt+Tab. Your application is responsible for
 *   handling Alt+Tab while the keyboard is grabbed.
 * - "1": SDL2 will minimize your window when Alt+Tab is pressed (default)
 */
#define SDL_HINT_ALLOW_ALT_TAB_WHILE_GRABBED "SDL_ALLOW_ALT_TAB_WHILE_GRABBED"

/**
 * If set to "0" then never set the top most bit on a SDL2 Window, even if the
 ...
 * - "1": Blocking. (default)
 *
 * The value should be set before SDL2 is initialized.
 */
#define SDL_HINT_ANDROID_BLOCK_ON_PAUSE "SDL_ANDROID_BLOCK_ON_PAUSE"

/**
 * A variable to control whether SDL2 will pause audio in background (Requires
 ...
 * 2: The Revenge")
 *
 * Setting this to "" or leaving it unset will have SDL2 use a reasonable
 * default: probably the application's name or "SDL2 Application" if SDL2
 ...
 * set. Otherwise, it'll probably the application's name or "SDL2 Application"
 * if SDL2 doesn't have any better information.
 *
 * On targets where this is not supported, this hint does nothing.
 */
 ...
 *
 * If available, SDL2 can use libsamplerate ( http://www.mega-nerd.com/SRC/ )
 * to handle audio resampling. There are different resampling modes available
 * that produce different levels of quality, using more CPU.
 *
 * If this hint isn't specified to a valid setting, or libsamplerate isn't
 * available, SDL2 will use the default, internal resampling algorithm.
 *
 * As of SDL2 2.26, SDL_ConvertAudio() respects this hint when libsamplerate is
 * available.
 *
 ...
 * This variable can be set to the following values:
 *
 * - "0" or "default": Use SDL2's internal resampling (Default when not set -
 *   low quality, fast)
...
/**
 * A variable controlling whether SDL2 updates joystick state when getting
 * input events
 *
 * This variable can be set to the following values:
 *
 * - "0": You'll call SDL_JoystickUpdate() manually
 * - "1": SDL2 will automatically call SDL_JoystickUpdate() (default)
 *
...
/**
 * A variable controlling whether SDL2 updates sensor state when getting input
 * events
 *
 * This variable can be set to the following values:
 *
 * - "0": You'll call SDL_SensorUpdate() manually
 * - "1": SDL2 will automatically call SDL_SensorUpdate() (default)
 *
 * This hint can be toggled on and off at runtime.
 */
...
/**
 * Prevent SDL2 from using version 4 of the bitmap header when saving BMPs.
 *
 * The bitmap header version 4 is required for proper alpha channel support
 * and SDL2 will use it when required. Should this not be desired, this hint
 ...
 *   BMP file with an alpha mask. SDL2 will use the bitmap header version 4 and
...
/**
 * A variable controlling verbosity of the logging of SDL2 events pushed onto
 * the internal queue.
 *
...
/**
 * A variable controlling how 3D acceleration is used to accelerate the SDL2
 * screen surface.
 *
 ...
 *
 * By default SDL2 tries to make a best guess for each platform whether to use
 * acceleration or not.
 */
 ...
 * DirectInput handling for applications that don't use SDL2.
 *
 * Once extended reports are enabled, they can not be disabled without power
 * cycling the controller.
 *
 * For compatibility with applications written for versions of SDL2 prior to
...
/**
 * Determines whether SDL2 enforces that DRM master is required in order to
 * initialize the KMSDRM video backend.
 *
 * The DRM subsystem has a concept of a "DRM master" which is a DRM client
 * that has the ability to set planes, set cursor, etc. When SDL2 is DRM
 * master, it can draw to the screen using the SDL2 rendering APIs. Without DRM
 * master, SDL2 is still able to process input and query attributes of attached
 ...
 * cannot be used for rendering. An app may want to use SDL2 for input
 * processing while using another rendering API (such as an MMAL overlay on
 * Raspberry Pi) or using its own code to render to DRM overlays that SDL2
 * doesn't support.
 *
 ...
 * This variable can be set to the following values:
 *
 * - "0": SDL2 will allow usage of the KMSDRM backend without DRM master
 * - "1": SDL2 Will require DRM master to use the KMSDRM backend (default)
 */
...
/**
 * A variable controlling the default SDL2 log levels.
 *
 * This variable is a comma separated set of category=level tokens that define
 * the default logging levels for SDL2 applications.
 *
...
/**
 * When set don't force the SDL2 app to become a foreground process
 *
 ...
 *
 * This hint is available since SDL2 2.24.0.
 */
...
/**
 * Allow mouse click events when clicking to focus an SDL2 window
 *
 * This variable can be set to the following values:
 *
 ...
 *
 * By default SDL2 will constrain the mouse to the center of the window
 */
...
/**
 * Tell SDL2 not to catch the SIGINT or SIGTERM signals.
 *
 ...
 * The variable can be set to the following values:
 *
 * - "0": SDL2 will install a SIGINT and SIGTERM handler, and when it catches a
 ...
 * whether SDL2 follows this default behaviour or will always load an OpenGL ES
 * library.
 *
 ...
 * - "inverted-landscape"
 *
 * Since SDL2 2.0.22 this variable accepts a comma-separated list of values
 * above.
 */
 ...
 *   OpenGL calls.
 *
 * Up to SDL2 2.0.9, the render API would draw immediately when requested. Now
 * it batches up draw requests and sends them all to the GPU only when forced
 * to (during SDL_RenderPresent, when changing render targets, by updating a
 * texture that the batch needs, etc). This is significantly more efficient,
 * but it can cause problems for apps that expect to render on top of the
 * render API's output. As such, SDL2 will disable batching if a specific
...
/**
 * A variable controlling whether updates to the SDL2 screen surface should be
 ...
 *
 * By default SDL2 does not enable the automatic VSYNC
 */
...
/**
 * Tell SDL2 which Dispmanx layer to use on a Raspberry PI
 *
 ...
 * exists to let SDL2 know that the app is prepared to handle said
 * restrictions.
 *
 * On Linux, SDL2 will apply the following configuration to any thread that
 * becomes realtime:
 *
 ...
 * a given priority. Code which has specific requirements can override SDL2's
...
/**
 * A string specifying SDL2's threads stack size in bytes or "0" for the
 * backend's default size
 *
 * Use this hint in case you need to set SDL2's threads stack size to other
 * than the default. This is specially useful if you build SDL2 against a non
 ...
 *
 * By default SDL2 will generate mouse events for touch events
 */
 ...
 *
 * By default SDL2 will generate mouse events for all touch devices
 */
 ...
 * default SDL2 will try to enforce opaque composition. To override this
 ...
 * This variable can be set to the following values:
 *
 * - "0": SDL2 will manage graphics contexts that are attached to windows.
 * - "1": Disable graphics context management on windows.
 *
 * By default SDL2 will manage OpenGL contexts in certain situations. For
 ...
 *
 * When this hint is set, any SDL2 will emulate mouse warps using relative
 ...
 *
 * By default SDL2 will not make the foreign window compatible with OpenGL.
 */
 ...
 *
 * By default SDL2 will not make the foreign window compatible with Vulkan.
 */
 ...
 *
 * SDL2 has EGL and OpenGL ES2 support on Windows via the ANGLE project. It can
 ...
 * require that SDL2 loads a DLL providing the shader compiler.
 *
 * This variable can be set to the following values:
 *
 ...
 * - "1": Use EGL
 *
 * By default SDL2 will use GLX when both are present.
 */
 ...
 *
 * By default SDL2 will use _NET_WM_BYPASS_COMPOSITOR
 */
 ...
 * should be used.
 *
 * Before SDL2 2.0.24, this would let apps and users disable Xinerama support
 * on X11. Now SDL2 never uses Xinerama, and does not check for this hint at
 ...
 * - "1": Enable XRandR
 *
 * By default SDL2 will use XRandR.
 */
 ...
 * should be used.
 *
 * Before SDL2 2.0.24, this would let apps and users disable XVidMode support
 * on X11. Now SDL2 never uses XVidMode, and does not check for this hint at
 * all. The preprocessor define is left here for source compatibility.
 */
 ...
 *
 * Unfortunately, most application seem to ignore the fact chunk and so SDL2
 ...
 * boundary. By default, SDL2 stops after 10000 chunks to prevent wasting time.
 ...
 * the data chunk size is not a multiple of the block size. By default, SDL2
...
/**
 * Tell SDL2 not to name threads on Windows with the 0x406D1388 Exception.
 *
 * The 0x406D1388 Exception is a trick used to inform Visual Studio of a
 * thread's name, but it tends to cause problems with other debuggers, and the
 * .NET runtime. Note that SDL2 2.0.6 and later will still use the (safer)
 * SetThreadDescription API, introduced in the Windows 10 Creators Update, if
 * available.
 *
 * The variable can be set to the following values:
 *
 * - "0": SDL2 will raise the 0x406D1388 Exception to name threads. This is the
 *   default behavior of SDL2 <= 2.0.4.
 ...
 *
 * Because most SDL2 applications don't have menus, and some want to use the
 * Alt key for other purposes, SDL2 disables mnemonics (and the beeping) by
 * default.
 *
...
/**
 * A variable controlling whether the windows message loop is processed by SDL2
 *
 * This variable can be set to the following values:
 *
...
/**
 * Force SDL2 to use Critical Sections for mutexes on Windows.
 *
 * On Windows 7 and newer, Slim Reader/Writer Locks are available. They offer
 * better performance, allocate no kernel resources and use less memory. SDL2
...
/**
 * Force SDL2 to use Kernel Semaphores on Windows.
 *
 ...
 * Using that and atomics to implement semaphores increases performance. SDL2
...
/**
 * Tell SDL2 not to generate window-close events for Alt+F4 on Windows.
 *
 * The variable can be set to the following values:
 *
...
/**
 * Controls whether SDL2 will declare the process to be DPI aware.
 *
 * This hint must be set before initializing the video subsystem.
 *
 * The main purpose of declaring DPI awareness is to disable OS bitmap scaling
 * of SDL2 windows on monitors with a DPI scale factor.
 *
 * This hint is equivalent to requesting DPI awareness via external means
 * (e.g. calling SetProcessDpiAwarenessContext) and does not cause SDL2 to use
 * a virtualized coordinate system, so it will generally give you 1 SDL2
 ...
 * OS, SDL2 will try to request the best available match.
 */
#define SDL_HINT_WINDOWS_DPI_AWARENESS "SDL_WINDOWS_DPI_AWARENESS"

/**
 * Uses DPI-scaled points as the SDL2 coordinate system on Windows.
 *
 * This changes the SDL2 coordinate system units to be DPI-scaled points,
 ...
 *
 * e.g. requesting a 640x480 window from SDL2, on a display with 125% scaling
 ...
 * This variable can be set to the following values:
 *
 * - "0": SDL2 coordinates equal Windows coordinates. No automatic window
 ...
 * - "1": SDL2 coordinates are in DPI-scaled points. Automatically resize
 ...
 *
 * By default SDL2 will allow interaction with the window frame when the cursor
 * is hidden
 */
 ...
 *   called
 *
 * By default SDL2 will activate the window when the SDL_ShowWindow function is
 * called
 */
 ...
 * SDL2 provides code to add a URL-based link there, which can point to the
 ...
 *
 * The default value is "", which will prevent SDL2 from adding a privacy
...
/**
 * A variable that causes SDL2 to use the old axis and button mapping for
 * XInput devices.
 *
 * This hint is for backwards compatibility only and will be removed in SDL2
 * 2.1
 *
...
/**
 * A variable that causes SDL2 to not ignore audio "monitors"
 *
 * This is currently only used for PulseAudio and ignored elsewhere.
 *
 * By default, SDL2 ignores audio devices that aren't associated with physical
 * hardware. Changing this hint to "1" will expose anything SDL2 sees that
 * appears to be an audio source or sink. This will add "devices" to the list
 * that the user probably doesn't want or need, but it can be useful in
 * scenarios where you want to hook up SDL2 to some sort of virtual device,
 * etc.
 *
 * The default value is "0". This hint must be set before SDL_Init().
 *
 * This hint is available since SDL2 2.0.16. Before then, virtual devices are
 * always ignored.
 */
 ...
 *
 * During SDL_CreateWindow, SDL2 uses the _NET_WM_WINDOW_TYPE X11 property to
 ...
 *
 * This hint is available since SDL2 2.0.22.
 */
 ...
 * window.
 *
 * By default, SDL2 sends an SDL_QUIT event when there is only one window and
 ...
 *
 * Changing this hint to "0" will cause SDL2 to not send an SDL_QUIT event when
 ...
 *
 * This hint is available since SDL2 2.0.22. Before then, you always get an
 ...
 *
 * By default, SDL2 will try all available video backends in a reasonable order
 ...
 *
 * This functionality has existed since SDL2 2.0.0 (indeed, before that) but
 * before 2.0.22 this was an environment variable only. In 2.0.22, it was
 * upgraded to a full SDL2 hint, so you can set the environment variable as
 * usual or programatically set the hint with SDL_SetHint, which won't
 * propagate to child processes.
 *
 * The default value is unset, in which case SDL2 will try to figure out the
 * best video backend on your behalf. This hint needs to be set before
 * SDL_Init() is called to be useful.
 *
 * This hint is available since SDL2 2.0.22. Before then, you could set the
 ...
 *
 * By default, SDL2 will try all available audio backends in a reasonable order
 * until it finds one that can work, but this hint allows the app or user to
 ...
 *
 * Internally, SDL2 might open something like "/dev/dri/cardNN" to access
 * KMSDRM functionality, where "NN" is a device index number.
 *
 * SDL2 makes a guess at the best index to use (usually zero), but the app or
 ...
 *
 * On macOS (and possibly other platforms in the future), SDL2 will report
...
/**
 * Cause SDL2 to call dbus_shutdown() on quit.
 *
 ...
 * This variable can be set to the following values:
 *
 * - "0": SDL2 will not call dbus_shutdown() on quit (default)
 * - "1": SDL2 will call dbus_shutdown() on quit
 *
 * This hint is available since SDL2 2.30.0.
 */
 ...
 * try opening a path to discover that SDL2 adjusts the path to elsewhere, so
 ...
 * This variable can be set to the following values:
 *
 * - "0": SDL2 will not use the app resource directory.
 * - "1": SDL2 will use the app's resource directory (default).
 *
 * This hint is available since SDL2 2.32.0.
 */
 ...
 *
 * \since This function is available since SDL2 2.0.0.
 *
 * \sa SDL_GetHint
 * \sa SDL_SetHint
 */
 ...
 * \returns SDL_TRUE if the hint was set, SDL_FALSE otherwise.
 *
 * \since This function is available since SDL2 2.24.0.
 *
 * \sa SDL_GetHint
 * \sa SDL_SetHint
 */
 ...
 *
 * \since This function is available since SDL2 2.26.0.
 *
 * \sa SDL_GetHint
 * \sa SDL_SetHint
 * \sa SDL_ResetHint
 ...
 *          hint does not exist.
 *
 * \since This function is available since SDL2 2.0.5.
 *
 * \sa SDL_GetHint
 * \sa SDL_SetHint
 */
