...
int main(void) {
    ...
    std::cout << "TESTING MAZE FILE!!" << std::endl;
    std::cout << "Test 1, no X found" << std::endl;
    ...
    std::cout << "Test 2, missing elements in maze (correctable)" << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(5000));
    ...
    std::cout << "Test 3, unsolvable maze" << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(5000));
    ...
    while (!isFinished) {
        ...
        // For testing, this is set to 1000 instead of 10000
        if (steps > 1000) {
            std::cout << "unsolvable maze detected" << std::endl;
            break;
        }
